...
def load_data(driver, db_name):
    print(f"\n--- Starting Data Load for {db_name} ---")
    
    with driver.session() as session:
        print("Clearing existing data...")
        run_query(session, "MATCH (n) DETACH DELETE n")
        
        print("Creating index on Person(name)...")
        run_query(session, "CREATE INDEX IF NOT EXISTS FOR (p:Person) ON (p.name)")
        
        print("Creating index on Person(id) to speed up loading...")
        run_query(session, "CREATE INDEX IF NOT EXISTS FOR (p:Person) ON (p.id)")
        ...
        nodes_time = end_time_nodes - start_time_nodes
        ...
        end_time_rels = time.time()
        rels_time = end_time_rels - start_time_rels
        rels_per_sec = len(rels) / rels_time
        
        total_time = nodes_time + rels_time
        
        print(f"\n--- {db_name} Load Metrics ---")
        print(f"Total time: {total_time:.2f} s | Nodes/s: {nodes_per_sec:.2f} | Rels/s: {rels_per_sec:.2f}")
        
        # Save to CSV
        save_metrics_to_csv(db_name, total_time, nodes_per_sec, rels_per_sec)

if __name__ == "__main__":
    cognodb_uri = os.environ.get("CONGODB_CONNECTION_URI")
    cognodb_user = os.environ.get("CONGODB_USERNAME")
    cognodb_pass = os.environ.get("CONGODB_PASSWORD")

    if cognodb_uri and cognodb_pass:
        driver = GraphDatabase.driver(cognodb_uri, auth=(cognodb_user, cognodb_pass))
        load_data(driver, "CognoDB")
        driver.close()
    else:
        print("CognoDB credentials not found in .env")
